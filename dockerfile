...
WORKDIR /opt
RUN wget https://dlcdn.apache.org/tomcat/tomcat-9/v9.0.58/bin/apache-tomcat-9.0.58-windows-x64.zip
RUN unzip apache-tomcat-9.0.58-windows-x64.zip
RUN mv apache-tomcat-9.0.58 tomcat
RUN chmod -R 700 tomcat
WORKDIR /opt
RUN git clone https://github.com/barmasairam/k8s-petclinic.git
